...
function loadPage(e) {
    ...
    if (t.innerHTML = "", n.style.display = "block", "nav-search" === e) return i.innerText = "Search", o.style.display = "flex", n.style.display = "none", SpatialNavigation.refresh(), void (a && setTimeout((function () {
        a.focus()
    }), 100));
    ...
    o.style.display = "none", "nav-home" === e ? (i.innerText = "Home", OdyseeAPI.getHome(r)) : (i.innerText = "Trending", OdyseeAPI.getTrending(r))
}
    ...
    t.innerHTML = "", n.style.display = "block", OdyseeAPI.search(e, (function (e, i) {
        ...
        if (i && i.items && i.items.length > 0)
            for (var a = 0; a < i.items.length; a++) {
                var r = createVideoCard(i.items[a]);
                r && t.appendChild(r)
            } else t.innerHTML = '<div style="color:white;text-align:center;width:100%;font-size:24px;margin-top:50px;">No results found.</div>';
...
function createVideoCard(e) {
    ...
    return o.className = "video-card focusable", o.innerHTML = '<div class="thumbnail-wrapper"><div class="thumbnail" style="background-image: url(\'' + n + '\')"></div></div><div class="info"><div class="title">' + t + '</div><div class="channel">' + i + "</div></div>", o.addEventListener("click", (function () {
        playVideo(e)
    })), o
}
