...
var Auth = (function () {
    ...
    function loadPersistedSession() {
        ...
        try {
            ...
            u = localStorage.getItem('odysee_auth_user');
            if (u) {
                state.user = JSON.parse(u);
            }

            internalTok = localStorage.getItem('odysee_internal_auth_token');
            if (internalTok) {
            ...
            s = localStorage.getItem('odysee_auth_settings');
            if (s) {
                parsedS = JSON.parse(s);
            ...
            m = localStorage.getItem('odysee_auth_memberships');
            if (m) {
                state.memberships = JSON.parse(m);
            }

            p = localStorage.getItem('odysee_auth_purchases');
            if (p) {
                state.purchases = JSON.parse(p);
            }
        } catch (e) {
    ...
    function refreshToken(cb) {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    notifyListeners();
                    if (cb) {
                        cb(new Error('Token refresh rejected: ' + xhr.status));
                    }
                }
            }
        };
        ...
        xhr.ontimeout = function () {
            ...
            if (cb) {
                cb(new Error('Token refresh network failure'));
            }
        };
    ...
    function fetchOdyseeProfile(cb) {
        LbryIo.call('/user/me', { method: 'POST' }, function (err, res) {
            ...
            if (!err && res && res.data) {
                d = res.data;
                if (d.primary_email && !state.user.email) {
                    state.user.email = d.primary_email;
                }
                if (d.auth_token) {
    ...
    function fetchUserInfo(cb) {
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    try {
                        ...
                        if (data.picture) {
                            state.user.avatarUrl = data.picture;
                        }
                    ...
                    if (cb) {
                        cb(new Error('Userinfo failed: ' + xhr.status));
                    }
                }
            }
        };
        ...
        xhr.ontimeout = function () {
            if (cb) {
                cb(new Error('Userinfo network error'));
            }
        };
        xhr.onerror = function () {
            if (cb) {
    ...
    function fetchUserChannel(cb) {
        LbryRpc.call('channel_list', { page_size: 20 }, function (err, result) {
            ...
            if (!err && result && result.items && result.items.length > 0) {
                ...
                for (ci = 0; ci < result.items.length; ci++) {
    ...
    function fetchMembershipsAndPurchases() {
        ...
        LbryIo.call('/membership_v2/list', { method: 'GET' }, function (err, res) {
            ...
            if (!err && res && res.data) {
                ...
                for (i = 0; i < items.length; i++) {
                    if (items[i].channel_id) {
                        state.memberships.push(items[i].channel_id);
                    }
                }
                savePersistedSession();
            }
        ...
        LbryIo.call('/purchase/list', { method: 'GET' }, function (err, res) {
            ...
            if (!err && res && res.data) {
                ...
                for (i = 0; i < items.length; i++) {
                    if (items[i].claim_id) {
                        state.purchases.push(items[i].claim_id);
                    }
                }
                savePersistedSession();
            }
    ...
    return {
        ...
        isMemberOf: function (channelClaimId) {
            if (!channelClaimId || !state.memberships) {
                return false;
            }
        ...
        startDeviceFlow: function (onCodeReady, onAuthenticated, onError) {
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        try {
                            ...
                            verificationUri = data.verification_uri || 'https://odysee.com/$/activate';
                            interval = (data.interval || 5) * 1000;
                            expiresIn = (data.expires_in || 600) * 1000;
                            ...
                            pollingTimer = setInterval(function () {
                                ...
                                if (Date.now() > expiresAt) {
                                    ...
                                    pollingTimer = null;
                                    if (onError) {
                                        onError(new Error('Activation code expired. Please request a new code.'));
                                    }
                                    return;
                                }
                                ...
                                pollXhr.onreadystatechange = function () {
                                    ...
                                    if (pollXhr.readyState === 4) {
                                        if (pollXhr.status === 200) {
                                            ...
                                            pollingTimer = null;
                                            try {
                                                tokenData = JSON.parse(pollXhr.responseText);
                                            ...
                                            try {
                                                errObj = JSON.parse(pollXhr.responseText);
                                                if (errObj.error === 'slow_down') {
                                                    ...
                                                    pollingTimer = null;
                                                    if (onError) {
                                                        onError(new Error(errObj.error_description || errObj.error));
                                                    }
                                                }
                                            } catch (e) { }
                                        }
                        ...
                        onError(new Error('Failed to initiate device flow: ' + xhr.status));
                    }
                }
            };
            ...
            xhr.ontimeout = function () {
                if (onError) {
                    onError(new Error('Network error connecting to SSO server'));
                }
            };
            xhr.onerror = function () {
                if (onError) {
        ...
        logout: function (cb) {
            ...
            if (state.accessToken) {
                ...
                xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                xhr.timeout = 5000;
                body = 'token=' + encodeURIComponent(state.accessToken) +
